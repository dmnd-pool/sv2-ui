...
function initialStep(account: DmndSession): OnboardingStep {
 ...
 * then add a payout address. The account state is read once on mount (the 2FA
 * secret rotates on every authed call, so we must not refetch between showing
 * the QR and activating). The connect-workers step lands with the home PR.
 */
export function Onboarding() {
  ...
  if (loadError) {
    return (
      <OnboardingLayout current="twofa" onBack={goHome}>
  ...
  return (
    <OnboardingLayout current={step} onBack={goHome}>
      {step === 'twofa' && account && (
        <Setup2faStep
 ...
 * Continue finish onboarding. Tokens come from the account loaded on mount.
 */
